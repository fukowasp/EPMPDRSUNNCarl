...
<head>
  ...
  <title>Training Categories | SUNN</title>
...
<main>
  <div class="container-fluid my-4">
    ...
    <div class="card p-3 shadow-sm">
      <table id="categoriesTable" class="table table-striped table-bordered nowrap align-middle w-100">
        <thead>
          <tr>
            <th>ID</th>
            <th>Category Name</th>
...
$(function () {
  ...
  const table = $('#categoriesTable').DataTable({
    responsive: true,
    ajax: {
      url: '<?= base_url("pdc/trainingcategories/fetch") ?>',
      type: 'GET',
      dataSrc: 'data',
    ...
    columns: [
      ...
      { data: 'category_name' },
      {
        ...
        render: (data, type, row) => `
          <button class="btn btn-sm btn-warning editBtn" data-id="${row.id}" data-name="${row.category_name}">
  ...
  $('#categoryForm').on('submit', function (e) {
    ...
    const id = $('#id').val();
    const url = id
      ? '<?= base_url("pdc/trainingcategories/update") ?>'
      : '<?= base_url("pdc/trainingcategories/store") ?>';
  ...
  $('#categoriesTable').on('click', '.deleteBtn', function () {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        $.post('<?= base_url("pdc/trainingcategories/delete") ?>', { id, _csrf_token: csrfToken }, res => { // ✅ include token
          const r = typeof res === 'string' ? JSON.parse(res) : res;
          Swal.fire({
            icon: r.status,
            title: r.message,
